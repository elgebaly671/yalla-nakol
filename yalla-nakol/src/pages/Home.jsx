...
const Home = () => {
  ...
  return (
    <div className='min-h-screen bg-slate-100'>
              ...
              <tbody>
                {sessions.length > 0 ? sessions.map((session) => (
                  <tr key={session.id} className='bg-slate-300/75 mb-20'>
                    ...
                    <td className='p-2'>
                      <button title='Enter Session' className='text-green-700 hover:text-green-800 transition-all cursor-pointer' onClick={() => navigate(`session/${session.id}`)}>
                        <ImEnter className='w-5 h-5' />
                      </button></td>
                  </tr>
                )) : <tr>
      ...
      <SessionForm
        isVisible={showForm1} onClose={() => setShowForm(false)}> 
        <form onSubmit={handleCreateSession} className='flex flex-col items-center gap-2 justify-center mb-4 w-full'>
          <input type="text" name="title" id="" placeholder='Session Title' className='w-full border border-slate-600 rounded-lg px-4 py-2' required />
          <input type="text" name="userName" placeholder='Your Name' className='w-full border border-slate-600 rounded-lg px-4 py-2' required />
          <button type="submit" className='bg-slate-800 text-white px-4 py-2 rounded-lg w-full hover:bg-slate-600 transition-colors duration-200 cursor-pointer'>Create</button>
        </form>
      </SessionForm>
    </div>
  )
